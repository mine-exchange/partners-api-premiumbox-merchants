...
class paymerchant_mine_partners extends Ext_AutoPayut_Premiumbox
{
    ...
    {
        $options = pn_array_unset($options, ['error_status', 'checkpay', 'note', 'enableip']);
    ...
    public function do_auto_payouts($error, $payError, $paymerchantId, $order, $place, $directionData, $paymerchantData, $unmetas, $modulePlace, $direction, $isTest, $paymerchantSettings)
    {
        $isTest = 0;
        if (!empty($error)) {
            $this->reset_ap_status($error, $payError, $order, $place, $paymerchantId, $isTest);
            return;
        }
        ...
        if (!isset($this->internalAccountCurrencies[$order->currency_code_get])) {
            ...
            $this->reset_ap_status([$commentText], 1, $order, $place, $paymerchantId, $isTest);
            return;
        }
        ...
        if (!$order->trans_out) {
            ...
            try {
                ...
                $this->reset_ap_status([$e->getMessage()], 1, $order, $place, $paymerchantId, $isTest);
                return;
            }
