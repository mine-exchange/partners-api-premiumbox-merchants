...
trait MinePartnersTrait
{
    ...
    protected function log($type, $message, $orderId = '')
    {
        if ($type == 'merchant') {
            $this->merchantLog($message);
        } else {
            $this->paymerchantLog($message, $orderId);
        }
    }

    protected function merchantLog($message)
    {
        do_action('merchant_logs', $this->name, '', $message);
    }

    protected function paymerchantLog($message, $orderId = '')
    {
        do_action('paymerchant_logs', $this->name, '', $message, $orderId);
    }
    ...
    protected function addAdminComment($text, $orderId, $commentsList = [])
    {
    ...
    protected function removeAdminCommentIfExists($commentText, $orderId)
    {
        if (empty($commentsList)) {
            $commentsList = $this->getAdminCommentsList($orderId);
        }

        $adminCommentId = $this->findInAdminCommentsList($commentText, $orderId, $commentsList);
