...
class merchant_mine_partners extends Merchant_Premiumbox
{
    ...
    public function bidaction($content, $merchantId, $amount, $order, $direction)
    {
        ...
        if (!isset($this->internalAccountCurrencies[$order->currency_code_give])) {
            $commentText = "Валюта \"{$order->currency_code_give}\" не поддерживается. Доступные: " . implode(',', array_keys($this->internalAccountCurrencies));
            $this->addAdminComment($commentText, $order->id);
            $this->merchantLog($commentText);
        ...
        if ($order->trans_in) {
            ...
            $commentText = 'Отсутствует вариант приёма оплаты. Свяжитесь с технической поддержкой платёжного шлюза';
            $this->addAdminComment($commentText, $order->id);
            $this->merchantLog($commentText);
            return __('Error! Please contact website technical support', 'pn');
        }
    ...
    public function cron($merchantId, $merchantSettings, $merchantData)
    {
        ...
        foreach ($ordersIds as $apiOrderId => $orderId) {
            if (!isset($ordersFromApi[$apiOrderId])) {
                ...
                $this->merchantLog("Error: Order (\"{$orderId}\") not found in response from api");
                continue;
            }
    ...
    public function webhook()
    {
        ...
        do_action('merchant_logs', $this->name, $orderData, $merchantId, $merchantSettings, $merchantData);
